...
\begin{figure*}[t]
\centering
\fbox{\rule{0pt}{4.5cm}\rule{15cm}{0pt}}
\caption{\textbf{Attractor basins and backward trajectories in cell state
space.}  \emph{Left:} two-dimensional schematic of the cell state space.  The
healthy attractor $\mathcal{A}_H$ (solid circle) and its basin $\mathcal{B}_H$
(light shading); the diseased attractor $\mathcal{A}_D$ (open circle) and
basin $\mathcal{B}_D$ (dark shading).  An observed state $\mathbf{x}^*$ (cross)
lies near the basin boundary.  \emph{Centre:} forward simulation from
$\mathbf{x}^*$ is bistable: trajectories starting from nearby initial
conditions split between the two attractors, so forward simulation cannot
resolve the classification.  This illustrates epistemic blindness.
\emph{Right:} the MAP backward trajectory (bold arrow) from $\mathbf{x}^*$
definitively identifies the origin basin as $\mathcal{B}_D$, enabling
disease detection.  The categorical address encodes this classification as a
time-invariant property of the observed state.}
...
\begin{figure*}[t]
\centering
\fbox{\rule{0pt}{4cm}\rule{15cm}{0pt}}
\caption{\textbf{Cellular time hierarchy and partition lags.}  A logarithmic
time axis spanning twelve decades from $10^{-12}$\,s (electronic transitions)
to $10^4$\,s (cell-cycle events).  Horizontal bands indicate the partition-lag
ranges of distinct reaction classes: electron transfer (ps), proton hopping
(ns--$\mu$s), enzymatic turnover ($\mu$s--ms), metabolic flux adjustments
(ms--s), gene regulation (min--h), and cell division (h).  Each class
constitutes an independent clock subsystem within the cellular circuit.  The
multi-timescale reduction of Theorem~\ref{thm:timescale} applies at each
boundary, with QSS values from faster timescales serving as parameters for
slower ones.  The trajectory completion algorithm can be applied independently
on each timescale.}
...
Figure~\ref{fig:drug-design} illustrates the drug design workflow.

\begin{figure*}[t]
\centering
\fbox{\rule{0pt}{4cm}\rule{15cm}{0pt}}
\caption{\textbf{Drug design as backward-trajectory restoration.}
\emph{Left:} circuit diagram of a diseased metabolic subnetwork.  Altered edges
(thick) have reduced conductances; the affected node (highlighted) has a
backward trajectory (red arrow) that escapes to the disease attractor
$\mathcal{A}_D$.  \emph{Centre:} solving~\eqref{eq:drug-opt} identifies the
minimal set of edges (circles) whose conductance modification restores the
backward trajectory to the healthy basin.  The $\ell_1$ objective ensures
only the most informative edges are targeted.  \emph{Right:} after applying
the optimal conductance modification $\eta^*$, the backward trajectory (green
arrow) converges to $\mathcal{A}_H$.  The drug rewires the circuit without
altering its topology---only selected edge weights change.}
\label{fig:drug-design}
\end{figure*}
