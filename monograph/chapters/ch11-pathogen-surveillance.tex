%% ch11-pathogen-surveillance.tex
%% Auto-converted from diseases\pathogen-biology\pathogen-biology-monitoring.tex into a monograph chapter.
%% Paper-local macros are provided via chapters/disease-macros.tex (loaded in preamble).

\chapter{Spectral Pathogen Surveillance}
\label{ps:ch:pathogen-surveillance}
...
Hamming distance (Proposition~\ref{ps:prop:lipschitz}), establishing that the
...
motifs under Gaussian noise (Theorem~\ref{ps:thm:optimal}). (3)~We derive the
...
Section~\ref{ps:sec:fingerprint} develops the spectral fingerprint embedding.
Section~\ref{ps:sec:identification} derives the identification matched filter and
proves optimality. Section~\ref{ps:sec:tropism} treats receptor--pathogen binding
and tropism prediction. Section~\ref{ps:sec:escape} addresses immune escape and
variant emergence. Section~\ref{ps:sec:resistance} covers antibiotic resistance.
Section~\ref{ps:sec:dynamics} models infection dynamics as oscillator coherence
decay. Section~\ref{ps:sec:microbiome} extends the framework to microbiome
dysbiosis. Section~\ref{ps:sec:gpu} describes the GPU architecture.
Section~\ref{ps:sec:validation} presents benchmarks. Section~\ref{ps:sec:discussion}
discusses limitations and related work. Section~\ref{ps:sec:conclusion} concludes.

% ============================================================
\section{Pathogen Sequences as Spectral Fingerprints}
\label{ps:sec:fingerprint}
% ============================================================
...
\begin{equation}
  ...
  \label{ps:eq:normalised_fingerprint}
\end{equation}
...
and added channel values and the change in the column mean~\eqref{ps:eq:mean_sub}.
...
\begin{remark}
Proposition~\ref{ps:prop:lipschitz} implies that the spectral fingerprint embeds
...
in~\eqref{ps:eq:crosscorr}. The normalisation in~\eqref{ps:eq:rho} is strictly
...
\begin{proof}
By Theorem~\ref{ps:thm:optimal}, the Neyman--Pearson test is uniformly most
...
\begin{figure*}[!htbp]
    ...
    (\textbf{C})~Exact position-recovery rate (fraction of trials in which the global $\arg\max$ of the normalised matched filter lies within $\pm 2$\,bp of the planted position) versus substitution rate $\mu$ for three query lengths $L_q\in\{50,\,100,\,200\}$\,bp, computed over $n=20$ trials per rate per query length. 
    (\textbf{D})~Three-dimensional surface of the analytically approximated peak $z$-score $z\approx\sqrt{L_q/50}\cdot\max(1-2\mu,\,0.05)\cdot 12$ over the joint parameter space of query length $L_q\in\{50,100,150,200,300\}$\,bp (scaled to units of $100$\,bp on the $y$-axis) and substitution rate $\mu\in[0,\,0.5]$, rendered in the \texttt{plasma} colormap. }
    \label{ps:fig:pathogen_panel1}
\end{figure*}
...
\For{each query read $q_i$}
  \State Channelise and mean-subtract: compute $\widetilde{Q}^{(i)}$ via~\eqref{ps:eq:onehot}--\eqref{ps:eq:mean_sub}
  \State Compute $\psi(q_i)$ via FFT using~\eqref{ps:eq:dft}--\eqref{ps:eq:normalised_fingerprint}
\EndFor
\For{each reference sequence $r_j$}
  \State Synthesise $\psi(r_j)$ on demand (no pre-stored database required)
\EndFor
\For{each pair $(i, j)$}
  \State Compute cross-correlation $\rho_{ij}$ via FFT convolution~\eqref{ps:eq:fft_xcorr}
...
Proposition~\ref{ps:prop:lipschitz} guarantees that, even when no exact reference
...
\item At sequence divergence $d_H/L \leq 0.20$: identification ROC-AUC
  $\geq 0.97$ (see Section~\ref{ps:subsec:id_bench}).
...
\begin{figure*}[!htbp]
    ...
    (\textbf{A})~Escape scores $E_i=-\rho(\psi_{v'_i},\psi_\text{Ab})+w\cdot\rho(\psi_{v'_i},\psi_\text{Rec})$ for the top 40 variants (ranked by $E_i$ descending) drawn from $n=300$ random perturbations of the wildtype viral protein spectrum, with fitness weight $w=0.8$, antibody neutralisation threshold $\theta_\text{neut}=0.25$, and receptor binding threshold $\theta_\text{bind}=0.05$. $\rho_\text{Rec}=\rho(\psi_{v'},\psi_\text{Rec})$ versus antibody recognition $\rho_\text{Ab}=\rho(\psi_{v'},\psi_\text{Ab})$ for all $n=300$ variants. The red dashed vertical line marks $\theta_\text{neut}=0.25$ and the blue dashed horizontal line marks $\theta_\text{bind}=0.05$; viable escape variants (green) occupy the upper-left quadrant. 
    (\textbf{C})~Compound escape difficulty: viable escape fraction plotted on a semi-logarithmic scale versus the number of antibodies in the neutralising panel (1--8), each drawn independently from the unit sphere in $\mathbb{R}^{48}$. 
    (\textbf{D})~Three-dimensional escape score surface: the escape score $E_i$ for $n=300$ variants is interpolated onto a $20\times 20$ regular grid over the first two spectral coordinates $(\psi_1,\psi_2)$ via linear Delaunay triangulation, rendered with the \texttt{RdYlGn} colormap. }
    \label{ps:fig:pathogen_panel3}
\end{figure*}

\subsection{Historical Validation Against SARS-CoV-2 Variants}

\noindent\textbf{Omicron escape mutations.} Applying
Algorithm~\ref{ps:alg:escape} to the ancestral Wuhan-Hu-1 SARS-CoV-2 Spike
...
landscape~\eqref{ps:eq:escape_landscape}, D614G has a high $E$ value primarily
...
$C(1/L)^{1/2}$ in $\ell^2$ norm (Proposition~\ref{ps:prop:lipschitz}). To reduce
...
Proposition~\ref{ps:prop:multiescape} provides a formal explanation for the
...
\begin{equation}
  ...
  \label{ps:eq:drug_binding}
\end{equation}

\begin{definition}[Resistance mutation]
\label{ps:def:resistance}
...
the drug was designed to inhibit. Mutations satisfying only~\eqref{ps:eq:resist_a}
but not~\eqref{ps:eq:resist_b} are enzymatically dead and are eliminated by
selection; mutations satisfying only~\eqref{ps:eq:resist_b} do not confer
resistance.

\begin{algorithm}
\caption{Resistance-robust drug candidate ranking}
\label{ps:alg:resistance}
\begin{algorithmic}[1]
...
catalytic activity on the beta-lactam ring. Applying Algorithm~\ref{ps:alg:resistance}
...
\begin{figure*}[!htbp]
    ...
    (\textbf{A})~Normalised histograms of drug-binding spectral overlap $\rho(\psi_\text{enzyme},\psi_\text{drug})$ for susceptible strains (green; $n=80$, sampled within standard deviation $\sigma=0.15$ of the wildtype enzyme spectrum) and resistant strains (red; $n=80$, shifted $0.5$ units away from the drug spectrum along the resistance direction).
    (\textbf{B})~Resistance mutation landscape: perturbation of drug binding $\Delta\rho_\text{drug}=\rho(\psi_{E_m},\psi_D)-\rho(\psi_E,\psi_D)$ versus perturbation of substrate binding $\Delta\rho_\text{sub}=\rho(\psi_{E_m},\psi_S)-\rho(\psi_E,\psi_S)$ for $n=50$ random enzyme mutations.
    (\textbf{C})~Robustness scores for $n_D=8$ drug candidates, defined as the product of the nominal binding score $\rho(\psi_D,\psi_E)$ and the worst-case binding retention $\min_{m\in\mathcal{M}_\text{res}}\rho(\psi_D,\psi_{E_m})$ across $n=50$ resistance mutations. 
    (\textbf{D})~Three-dimensional robustness surface over the joint space of drug-enzyme binding $\rho_\text{drug}\in[-0.5,\,1.0]$ and substrate-enzyme binding $\rho_\text{sub}\in[-0.5,\,1.0]$, defined as $\text{Robustness}=\rho_\text{drug}\cdot\max(\rho_\text{sub},\,0)$ on a $20\times 20$ grid, rendered in the \texttt{RdYlGn} colormap.}
    \label{ps:fig:pathogen_panel4}
\end{figure*}

% ============================================================
\section{Infection Dynamics and Host Oscillator Coherence}
\label{ps:sec:dynamics}
% ============================================================
...
limits are verified by direct inspection of the fixed points of~\eqref{ps:eq:coherence_decay}.
\end{proof}

The steady state of~\eqref{ps:eq:coherence_decay} at constant spectral load
...
\State Compute $\omega_{ij}$ for all $i \in [8]$, $j \in [N_P]$ via~\eqref{ps:eq:omega}
...
\For{each oscillator class $i \in [8]$}
  \State Integrate~\eqref{ps:eq:coherence_decay} via 4th-order Runge--Kutta
...
with high $\omega_{ij}$. The steady state~\eqref{ps:eq:steady_state} shows that
...
\begin{figure*}[!htbp]
    ...
    (\textbf{A})~Time courses of the disease index $D_i(t)=1-\eta_i(t)$ for all eight host oscillator classes $i\in\{$P, E, C, M, A, G, Ca, R$\}$ over a simulated $14$-day infection, obtained by numerical integration (RK45, relative tolerance $10^{-6}$) of the coherence decay ODE.
    (\textbf{B})~Spectral overlap coefficients $\omega_{ij}=\rho(\psi_{p_j},\psi_i^\text{host})$ for $j\in\{1,\ldots,5\}$ viral proteins (VP1--VP5) across the eight host oscillator classes, displayed as a grouped bar chart (bar width $0.08$, five groups per class). 
    (\textbf{C})~Peak disease index $D_i^* = \max_t D_i(t)$ as a function of viral load $V_\text{max}\in[0.1,\,2.0]$ for three oscillator classes: protein folding (P, blue), mitochondrial ATP (A, orange), and gene expression (G, purple), computed from $n=30$ ODE integrations at evenly spaced $V_\text{max}$ values. 
    (\textbf{D})~Three-dimensional disease surface: the full $8\times 200$ disease trajectory matrix $D_i(t_k)$ rendered as a surface with oscillator class on the $x$-axis (labelled P, E, C, M, A, G, Ca, R), time on the $y$-axis ($0$--$14$\,days), and $D_i(t)$ on the $z$-axis, coloured by the \texttt{coolwarm} colormap (blue$=0$, red$=1$). }
    \label{ps:fig:pathogen_panel5}
\end{figure*}

% ============================================================
\section{Microbiome Dysbiosis as Collective Spectral Interference}
\label{ps:sec:microbiome}
% ============================================================
...
\label{ps:def:microbiome_spectrum}
...
normalised problem~\eqref{ps:eq:probiotic_opt} is a quadratic program on the
...
\State \quad $\omega_{ij} \gets \langle \textsc{Synthesise}(p_j),\; \psi_i^{\text{host}} \rangle$;
  integrate~\eqref{ps:eq:coherence_decay} per class $i$
\end{algorithmic}
\end{algorithm}

\subsection{Scaling Analysis}

Table~\ref{ps:tab:scaling} summarises the per-query computational complexity and
...
% ============================================================
\section{Validation}
\label{ps:sec:validation}
% ============================================================

\subsection{Identification Benchmarks}
\label{ps:subsec:id_bench}
...
$(\mu, L_q)$ combination and apply Algorithm~\ref{ps:alg:identification} to
...
the theoretical prediction of $\sqrt{4} = 2$ from equation~\eqref{ps:eq:zchan}
...
The degradation curve is consistent with the Lipschitz bound~\eqref{ps:eq:lipschitz}:
...
\label{ps:subsec:escape_bench}

Applying Algorithm~\ref{ps:alg:escape} to the ancestral Wuhan-Hu-1 SARS-CoV-2
...
\label{ps:subsec:resist_bench}

Applying Algorithm~\ref{ps:alg:resistance} to TEM-1 beta-lactamase with a
...
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:id_zscore}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:id_recovery}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:id_gain}
\end{subfigure}
...
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:trop_auc}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:trop_heatmap}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:trop_scatter}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:trop_boundary}
\end{subfigure}
...
plane, illustrating the conjunctive threshold structure of~\eqref{ps:eq:multi_receptor}.}
\label{ps:fig:tropism}
\end{figure*}
...
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:esc_landscape}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:esc_omicron}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:esc_resist}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:esc_compound}
\end{subfigure}
...
Proposition~\ref{ps:prop:multiescape}.}
\label{ps:fig:escape}
\end{figure*}
...
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:dyn_trajectory}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:dyn_dominant}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:micro_dysbiosis}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.23\textwidth}
  \rule{\textwidth}{0.6\textwidth}
  \caption{}
  \label{ps:fig:micro_probiotic}
\end{subfigure}
...
dysfunction.
(\textbf{C})~Microbiome dysbiosis score $\Delta$~\eqref{ps:eq:dysbiosis} in
...
quadratic program~\eqref{ps:eq:probiotic_opt} to restore eubiosis from the
...
\citep{turnbaugh2006}.}
\label{ps:fig:dynamics}
\end{figure*}

% ============================================================
\section{Discussion}
\label{ps:sec:discussion}
% ============================================================

\subsection{Limitations}

\noindent\textbf{Insertions and deletions.} The matched-filter formulation of
Section~\ref{ps:sec:identification} addresses substitutions but not insertions or
...
Algorithm~\ref{ps:alg:resistance} should therefore be interpreted as a filter for
...
Section~\ref{ps:sec:validation} use synthetic data generated under the same
...
sequence assembly. The single-shader cascade architecture (Algorithm~\ref{ps:alg:gpu})
