%% ch12-immunology-primitive.tex
%% Auto-converted from diseases\universal-immunology-primitive\universal-immunology-primitive.tex into a monograph chapter.
%% Paper-local macros are provided via chapters/disease-macros.tex (loaded in preamble).

\chapter{Molecular Recognition as Oscillator Interference}
\label{im:ch:immunology-primitive}
...
\begin{enumerate}
  \item A formal derivation of molecular recognition as oscillator interference,
    ...
    linear detector (Theorem~\ref{im:thm:matched_filter}).
  \item Derivation of MHC-peptide binding, TCR activation, antibody
    ...
    (Section~\ref{im:sec:immunological_primitives}).
  \item The \emph{empty database principle}
    (Section~\ref{im:sec:empty_database}): spectra are synthesised on demand,
  ...
  \item Polynomial-time algorithms for neoantigen identification, vaccine
    ...
    (Section~\ref{im:sec:applications}).
  \item Synthetic validation benchmarks with ROC-AUC $\geq 0.97$ through 20\%
    sequence divergence (Section~\ref{im:sec:validation}).
\end{enumerate}

The paper is structured as follows. Section~\ref{im:sec:oscillator} develops the
spectral representation of biological sequences.
Section~\ref{im:sec:recognition} formalises recognition as interference and proves
optimality. Section~\ref{im:sec:immunological_primitives} derives the immunological
special cases. Section~\ref{im:sec:applications} presents the computational
applications. Section~\ref{im:sec:empty_database} analyses the empty database
principle. Section~\ref{im:sec:gpu} describes the GPU implementation.
Section~\ref{im:sec:validation} reports validation results.
Sections~\ref{im:sec:discussion} and~\ref{im:sec:conclusion} discuss limitations and
conclude.

%% ============================================================
\section{Biological Sequences as Oscillator Systems}
\label{im:sec:oscillator}

\subsection{Channelised sequence representation}
\label{im:sec:channelisation}
...
\begin{align}
  X_{1,i} &= h(s_i), \label{im:eq:hydropathy}\\
  X_{2,i} &= v(s_i), \label{im:eq:volume}\\
  X_{3,i} &= q(s_i), \label{im:eq:charge}
\end{align}
...
\begin{figure*}[!htbp]
    ...
    \label{im:fig:immuno_panel1}
\end{figure*}

\subsection{Spectral Lipschitz continuity}
\label{im:sec:lipschitz}
...
By Parseval's theorem applied to the DFT in \eqref{im:eq:dft},
\[
...
\begin{remark}
Proposition~\ref{im:prop:lipschitz} is why homologous sequences share dominant
...
\label{im:sec:recognition}

\subsection{The interference cross-term}
\label{im:sec:cross_term}
...
\begin{equation}
  ...
  \label{im:eq:parseval_innerprod}
\end{equation}
...
\end{align*}
The normalised matched filter $\rho(q,t)$ defined in \eqref{im:eq:rho} is the
...
\begin{equation}
  ...
  \right\},
  \label{im:eq:cross_correlation_fft}
\end{equation}
...
\label{im:sec:immunological_primitives}

We now show that the principal molecular recognition events in adaptive
immunology are all special cases of Definition~\ref{im:def:binding},
...
consequence of Definition~\ref{im:def:pmhc_binding}: the binding set is the
ball $\mathcal{B}(\bm{\Gamma}(M), r^*_{\mathrm{MHC}})$
(Corollary~\ref{im:cor:ball}), which, for $\rho^*_{\mathrm{MHC}} = 0.65$ and
...
\begin{equation}
  ...
  \label{im:eq:cross_reactivity_ball}
\end{equation}
...
\begin{figure*}[!htbp]
    ...
    \label{im:fig:immuno_panel3}
\end{figure*}

\begin{proof}
Immediate from Definition~\ref{im:def:tcr_activation} and
Corollary~\ref{im:cor:ball}: the set of pMHC complexes that activate the TCR
is the ball
$\{(p,M) : \bm{\Gamma}(\mathrm{TCR})\cdot\bm{\Gamma}(p,M) > \theta_{\mathrm{act}}\}$.
By Proposition~\ref{im:prop:lipschitz}, perturbations of $p$ that remain within
...
\begin{remark}
Theorem~\ref{im:thm:cross_reactivity} provides the theoretical foundation for
...
\label{im:thm:molecular_mimicry}
...
$\bm{\Gamma}(p, M_j)$ is a convex combination \eqref{im:eq:pmhc_composite}
...
threshold $\theta'$. If $f$ and $s$ satisfy \eqref{im:eq:mimicry}, then by the
...
proximity, and \eqref{im:eq:mimicry} is a precise geometric criterion for when
such cross-activation occurs.
\end{remark}

%% ============================================================
\section{Applications}
\label{im:sec:applications}

\subsection{Neoantigen identification}
\label{im:sec:neoantigen}
...
Algorithm~\ref{im:alg:escape} evaluates $20 \times L$ variants, each requiring
...
\label{im:sec:ab_cross_reactivity}
...
$\bm{\Gamma}(\mathrm{Ab})$ is computed via Definition~\ref{im:def:tcr_spectrum}
...
\begin{figure*}[!htbp]
    ...
    Each additional antibody erects a new spectral barrier that all viable variants must simultaneously evade; the viable fraction decreases approximately exponentially with panel size, explaining quantitatively why multi-epitope vaccines are more robust than mono-epitope formulations.
...
Section~\ref{im:sec:channelisation} [$\mathcal{O}(cL)$];
...
\begin{figure*}[!htbp]
    ...
    Stored structural coordinates (PDB-style, approximately $10^3$ atoms $\times$ 3 coordinates $\times$ 4\,bytes) require $\approx 12{,}000$\,MB; stored spectral embeddings ($d=48$ floats at 4\,bytes each) require $192$\,MB.
...
of Proposition~\ref{im:prop:decomp}. In the standard graphics pipeline, a
...
\begin{figure*}[!htbp]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Principal-component projection of the $d=48$-dimensional spectral
      ...
      separation confirms Proposition~\ref{im:prop:lipschitz}.}
    \label{im:fig:pca_embedding}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    ...
    \caption{ROC curves for the matched-filter binding detector at four levels
      ...
      baseline (AUC $= 0.5$). Results confirm Theorem~\ref{im:thm:matched_filter}.}
    \label{im:fig:roc}
  \end{subfigure}
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Distribution of cosine similarity $\rho(\bm{\psi}(p),
      ...
      on the synthetic benchmark, validating Definition~\ref{im:def:pmhc_binding}.}
    \label{im:fig:mhc_dist}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    ...
    \label{im:fig:cross_reactivity}
  \end{subfigure}

  \caption{Synthetic validation results. All benchmarks generated using the
    procedures described in Section~\ref{im:sec:validation}. No experimental data
  ...
  \label{im:fig:validation_panels}
\end{figure*}

%% ============================================================
\section{Validation}
\label{im:sec:validation}
...
(Figure~\ref{im:fig:roc}). The AUC decay is consistent with
Proposition~\ref{im:prop:lipschitz}: at $\mu = 0.20$, the expected spectral
...
(Figure~\ref{im:fig:mhc_dist}), consistent with the theoretical prediction that
...
random 9-mers are predicted cross-reactive (Figure~\ref{im:fig:cross_reactivity}).
...
sliding matched filter (Section~\ref{im:sec:sliding_filter}), phase is retained
...
Proposition~\ref{im:prop:lipschitz}. The standard remedy is to apply a
...
as the matched filter (Theorem~\ref{im:thm:matched_filter}), the Neyman-Pearson
...
(Theorem~\ref{im:thm:matched_filter}), requires no training data and no stored
structural coordinates (Proposition~\ref{im:prop:determinism} and
Corollary~\ref{im:cor:empty_db}), and runs at peak GPU throughput on commodity
...
Algorithms~\ref{im:alg:neoantigen}--\ref{im:alg:gpu}.
