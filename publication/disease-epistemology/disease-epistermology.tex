...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_1_floor_positivity.png}
    \caption{\textbf{Floor Positivity: the irreducible epistemic floor of a bounded receiver.}
    (\textbf{A})~Cellular floor $\beta = \sigma/\!\sqrt{N}$ versus $N$ (log--log).
    Blue circles are exact theoretical values; the grey dashed line is $N^{-1/2}$.
    All six points are strictly positive across six decades of $N$, confirming
    \Cref{thm:floor-positivity}.
    (\textbf{B})~Floor $\beta(\sigma)$ versus spread $\sigma$ at $N = 1000$ (linear axes).
    The filled region quantifies the irreducible uncertainty at each noise level.
    (\textbf{C})~Empirical (blue) versus theoretical (grey) floors from Poisson
    shot-noise Monte Carlo (5\,000 trials per $N$); relative error $< 0.7\%$ at all
    sample sizes (E05).
    (\textbf{D})~Three-dimensional surface $S(x_1, x_2;\Cell)$ over a two-dimensional
    state slice.
    Inside the action cell ($R=1$, $\beta=0.05$) the surface is a flat plateau at
    $S = \beta$; outside, $S$ rises as $\max(0,r-R)+\beta$.
    The plateau encodes Cell-Truth (\Cref{thm:cell-truth}): no pointwise discrimination
    exists within $\Cell$.}
    \label{fig:panel1}
\end{figure*}
...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_2_cell_truth.png}
    \caption{\textbf{Cell-Truth and representational invariance.}
    (\textbf{A})~Fraction of state space inside the action cell versus floor $\beta$
    for $\sigma \in \{0.1, 0.5, 1.0, 2.0\}$ at $N = 500$.
    Bubble area is proportional to the fraction; colour encodes $\sigma$.
    The monotone growth confirms that a larger floor implies a coarser, larger cell
    (\Cref{thm:cell-truth}).
    (\textbf{B})~Polar bar chart of success rates for three epistemic modes
    (knowledge, behaviour, belief) over 1\,000 agents each.
    All three modes achieve success rate $\in [0.939, 0.945]$, maximum spread $0.006$,
    confirming Mode Non-Privilege (\Cref{thm:mode-nonprovilege}; E10).
    (\textbf{C})~Distribution of $\mathrm{dist}(x,\Cell)$ for states outside the action cell.
    The vertical blue line at $\mathrm{dist}=0$ marks the cell boundary; all inside states
    have $S = \beta$ exactly (E06), while outside states have a continuous excess distance.
    (\textbf{D})~Three-dimensional S-landscape.
    The flat plateau (dark blue, $S = \beta$) is bounded by the cell edge (orange ring at
    $R=1$); $S$ rises linearly beyond.
    No interior point is distinguished, so no template can be stored
...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_3_group_blindness.png}
    \caption{\textbf{Group Blindness: multiplicative compounding of cellular epistemic floors.}
    (\textbf{A})~Composite organ floor $\Sfloor(E) = \beta^n/(n\beta)^{n-1}$ (log $y$)
    versus cell count $n = 1$ to $20$ for a homogeneous ensemble ($\beta_i = 0.1$).
    Every value is strictly positive; the floor decays super-exponentially but never
    vanishes (\Cref{thm:group-blindness}; E13).
    (\textbf{B})~$\Sfloor(E)$ (log--log) versus individual floor $\beta$ for a 10-cell
    ensemble.
    Purple circles track the formula; the grey dashed diagonal is the single-cell limit
    $\Sfloor = \beta$.
    The composite floor lies far below, quantifying organ-level blindness (E15).
    (\textbf{C})~Distribution of $\log_{10}\Sfloor(E)$ over 500 randomly drawn
    ensembles with $n \sim \mathrm{Uniform}\{2,\ldots,19\}$,
    $\beta_i \sim \mathrm{Uniform}[10^{-3}, 0.5]$.
    The distribution spans 28 orders of magnitude; all values are positive,
    confirming the universality of Group Blindness (E11).
    (\textbf{D})~Three-dimensional surface $\log_{10}\Sfloor(n, \beta)$ over
    $n \in \{2,\ldots,11\}$ and $\log_{10}\beta \in [-2, -0.3]$.
    The surface is everywhere finite and negative, consistent with
    $0 < \Sfloor \ll 1$ across the full parameter range.}
    \label{fig:panel3-blindness}
\end{figure*}
...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_4_purpose_attractor.png}
    \caption{\textbf{Purpose as Fixed-Point: the organ's healthy regime as an emergent attractor.}
    (\textbf{A})~Convergence of $\|x_t - x^*\|$ (log $y$) versus iteration $t$ for
    ensemble dynamics $x_{t+1} = Ax_t + b$ in $\mathbb{R}^5$, contraction ratio
    $\|A\| < 1/1.1$.
    The trajectory reaches machine precision ($< 10^{-14}$) within 65 iterations (E16).
    (\textbf{B})~Scatter of 200 fixed points $x^* = (I-A)^{-1}b$ under small goal
    perturbations ($\|\delta b\| \leq 10^{-3}$).
    Purple dots cluster tightly around the true fixed point (red disc),
    confirming Purpose Stability (\Cref{thm:purpose-stability}; E17).
    (\textbf{C})~Residual $\|x_\infty - x^*\|$ (log $y$) versus initial perturbation
    norm $\|\varepsilon\|$ (log $x$) across 30 perturbation scales from $10^{-3}$ to $1$.
    All residuals fall below $10^{-12}$ (grey dashed), confirming global convergence
    from any starting point (\Cref{thm:purpose-attractor}; E18).
    (\textbf{D})~Three-dimensional phase portrait: twelve trajectories from diverse
    initial conditions ($\|x_0 - x^*\| \approx 1.5$) converge to the fixed point (red disc)
    in the $(x_1, x_2, x_3)$ subspace, illustrating that purpose is an attractor, not
    a stored representation (\Cref{thm:purpose-existence}).}
    \label{fig:panel4}
\end{figure*}
...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_5_self_consistency.png}
    \caption{\textbf{Self-consistency superiority and sparse therapeutic design.}
    (\textbf{A})~ROC curves for template comparison (grey, AUC$=0.896$) and
    Wegscheider log-holonomy (green, AUC$=1.00$) applied to 200 healthy and
    200 diseased synthetic circuits.
    The holonomy test achieves perfect discrimination while requiring no stored reference
    (\Cref{thm:primacy}; E21).
    (\textbf{B})~Distribution of $\log_{10}|\HolL|$ for healthy (blue) and diseased (red)
    circuits.
    Healthy circuits cluster near machine precision ($\approx -15$); diseased circuits
    cluster near $0$ ($|\HolL| \approx O(1)$).
    The populations are completely separated (E19, E20).
    (\textbf{C})~$\ell_1$ LP solution $\drugvec^*$ for a representative 20-drug,
    4-loop circuit.
    Only 4 of 20 drug slots receive non-zero weight (sparsity $= 0.20$); remaining 16
    are zeroed, confirming the Sparse Therapeutic Optimum (\Cref{thm:sparse-therapeutic};
    E23).
    (\textbf{D})~Three-dimensional holonomy landscape $|\HolL(\Delta k_1, \Delta k_2)|$
    over rate-constant perturbation axes $\Delta k_1, \Delta k_2 \in [-2,2]$.
    The zero-holonomy valley at $(0,0)$ is the detailed-balance manifold; any deviation
    increases $|\HolL|$ monotonically.
    Therapeutic LP finds the minimum-norm perturbation restoring this valley.}
    \label{fig:panel5}
\end{figure*}
